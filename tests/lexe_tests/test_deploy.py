...
def test_deploy_run(tmp_path, monkeypatch, capsys):
    ...
    def fake_sub_run(*args, **kwargs):
        ...
        if args[-1] == 'ps':
            return CompletedProcess(
                args, 0, stdout='NAME SERVICE STATUS\nhello-web-1 web running(healthy)\n', stderr=''
            )
        return CompletedProcess(args, 0, stdout='', stderr='')
...
def test_deploy_run_requires_clean_git_worktree_by_default(tmp_path, monkeypatch):
    for name in ('Dockerfile', 'compose.yaml', 'compose.server.yaml'):
        (tmp_path / name).write_text('x')
...
def test_deploy_run_appends_dirty_suffix_when_allowed(tmp_path, monkeypatch, capsys):
    for name in ('Dockerfile', 'compose.yaml', 'compose.server.yaml'):
        (tmp_path / name).write_text('x')
